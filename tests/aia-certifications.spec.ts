...
const AIA_EXPECTED_CERTIFICATIONS = [
  {
...
test.describe('AIA Environment - Certification Dropdown Validation', () => {
  test('verify AIA certification dropdown shows all expected values', async ({ page }) => {
    // Very flexible timeout - allow up to 5 minutes for slow environments
    test.setTimeout(300000);
    const summary = {
      environment: 'AIA',
      status: 'pending',
      expectedCertifications: AIA_EXPECTED_CERTIFICATIONS.map((cert) => cert.title),
      expectedSubtitles: AIA_EXPECTED_CERTIFICATIONS.map((cert) => cert.subtitle),
    ...
    const summaryPath = path.resolve('report', 'aia-summary.json');
    ...
    console.log('AIA: Navigating to registration page...');
    ...
    try {
      console.log('AIA: Filling personal information...');
      ...
      console.log('AIA: Opening certification dropdown...');
      ...
      console.log('AIA Environment - Verifying certification dropdown values...');

      // Wait for certifications to load using polling approach (more reliable than networkidle)
      console.log('AIA: Waiting for certifications to load...');
      ...
      while (!certificationsLoaded && (Date.now() - startTime) < maxWaitTime) {
        ...
        if (validTitles.length > 0) {
          console.log(`AIA: Found ${validTitles.length} certifications after ${Math.round((Date.now() - startTime) / 1000)}s`);
      ...
      if (!certificationsLoaded) {
        throw new Error(
          'AIA certifications did not load: no valid <h3> titles found in dropdown within 180s. ' +
      ...
      const collectCertifications = async (retries = 5) => {
        for (let i = 0; i < retries; i++) {
          ...
          if (i < retries - 1) {
            console.log(`AIA: Retrying certification collection (attempt ${i + 1}/${retries})...`);
            await page.waitForTimeout(2000);
          }
        }
      };
      ...
      console.log(`📋 AIA - Found ${summary.actualCertifications.length} certification titles in dropdown`);
      console.log(`📋 AIA - Titles: ${summary.actualCertifications.join(', ')}`);
      console.log(`📋 AIA - Subtitles: ${summary.actualSubtitles.join(', ')}`);
      ...
      for (const cert of AIA_EXPECTED_CERTIFICATIONS) {
        console.log(`AIA: Checking for certification: ${cert.title}`);
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
          ...
          if (attempt < 4) {
            console.log(`AIA: Retrying title check for ${cert.title} (attempt ${attempt + 1}/5)...`);
            await page.waitForTimeout(2000);
          }
        }
        ...
        if (titleCount === 0) {
          ...
          await expect(
            titleLocator,
            `AIA certification missing: ${cert.title}\nTitles found: ${
          ...
          console.log(`✅ AIA Found: ${cert.title}`);
        }
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
          ...
          if (attempt < 4) {
            console.log(`AIA: Retrying subtitle check for ${cert.subtitle} (attempt ${attempt + 1}/5)...`);
            await page.waitForTimeout(2000);
          }
        }
        ...
        if (!subtitleVisible) {
          ...
          await expect(
            subtitleVisible,
            `AIA subtitle missing: ${cert.subtitle}\nSubtitles found: ${
              summary.actualSubtitles.length > 0 ? summary.actualSubtitles.join(', ') : 'none'
            }`
          ).toBeTruthy();
        } else {
          console.log(`✅ AIA Subtitle Found: ${cert.subtitle}`);
        }
      }

      // Test selecting ONE certification only
      console.log(`AIA Testing selection of: ${summary.expectedCertifications[0]}`);
      ...
      console.log(`✅ AIA Selected: ${summary.expectedCertifications[0]}`);

      // STOP HERE - NO MORE STEPS
      console.log('✅ AIA Certification dropdown validation complete - STOPPING HERE');

      // Take screenshot
      await page.screenshot({ path: 'report/aia-certification-dropdown-validated.png' });

      console.log('AIA Test completed - no registration attempted');
      summary.status = 'passed';
    } catch (error) {
      summary.status = 'failed';
      console.error('AIA Test failed:', error);
      throw error;
    } finally {
