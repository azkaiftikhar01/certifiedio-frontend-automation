...
const ETRAINING_STAGE_EXPECTED_CERTIFICATIONS = [
  {
...
test.describe('ETRAINING-STAGE Environment - Certification Dropdown Validation', () => {
  test('verify ETRAINING-STAGE certification dropdown shows all expected values', async ({ page }) => {
    // Very flexible timeout - allow up to 5 minutes for slow environments
    test.setTimeout(300000);
    const summary = {
      environment: 'ETRAINING-STAGE',
      status: 'pending',
      expectedCertifications: ETRAINING_STAGE_EXPECTED_CERTIFICATIONS.map((cert) => cert.title),
      expectedSubtitles: ETRAINING_STAGE_EXPECTED_CERTIFICATIONS.map((cert) => cert.subtitle),
    ...
    const summaryPath = path.resolve('report', 'etraining-stage-summary.json');
    ...
    console.log('ETRAINING-STAGE: Navigating to registration page...');
    ...
    try {
      console.log('ETRAINING-STAGE: Filling personal information...');
      ...
      console.log('ETRAINING-STAGE: Clicking Continue button...');
      await page.click('button:has-text("Continue")', { timeout: 60000 });

      // Wait for step 2 to load with very flexible timeout
      console.log('ETRAINING-STAGE: Waiting for step 2 to load...');
      ...
      console.log('ETRAINING-STAGE: Opening certification dropdown...');
      ...
      console.log('ETRAINING-STAGE Environment - Verifying certification dropdown values...');

      // Wait for certifications to load using polling approach (more reliable than networkidle)
      console.log('ETRAINING-STAGE: Waiting for certifications to load...');
      ...
      while (!certificationsLoaded && (Date.now() - startTime) < maxWaitTime) {
        ...
        if (validTitles.length > 0) {
          console.log(`ETRAINING-STAGE: Found ${validTitles.length} certifications after ${Math.round((Date.now() - startTime) / 1000)}s`);
      ...
      if (!certificationsLoaded) {
        throw new Error(
          'ETRAINING-STAGE certifications did not load: no valid <h3> titles found in dropdown within 180s. ' +
      ...
      const collectCertifications = async (retries = 5) => {
        for (let i = 0; i < retries; i++) {
          ...
          if (i < retries - 1) {
            console.log(`ETRAINING-STAGE: Retrying certification collection (attempt ${i + 1}/${retries})...`);
            await page.waitForTimeout(2000);
          }
        }
      };
      ...
      console.log(`📋 ETRAINING-STAGE - Found ${summary.actualCertifications.length} certification titles in dropdown`);
      console.log(`📋 ETRAINING-STAGE - Titles: ${summary.actualCertifications.join(', ')}`);
      console.log(`📋 ETRAINING-STAGE - Subtitles: ${summary.actualSubtitles.join(', ')}`);
      ...
      for (const cert of ETRAINING_STAGE_EXPECTED_CERTIFICATIONS) {
        console.log(`ETRAINING-STAGE: Checking for certification: ${cert.title}`);
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
          ...
          if (attempt < 4) {
            console.log(`ETRAINING-STAGE: Retrying title check for ${cert.title} (attempt ${attempt + 1}/5)...`);
            await page.waitForTimeout(2000);
          }
        }
        ...
        if (titleCount === 0) {
          ...
          await expect(
            titleLocator,
            `ETRAINING-STAGE certification missing: ${cert.title}\nTitles found: ${
          ...
          console.log(`✅ ETRAINING-STAGE Found: ${cert.title}`);
        }
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
          ...
          if (attempt < 4) {
            console.log(`ETRAINING-STAGE: Retrying subtitle check for ${cert.subtitle} (attempt ${attempt + 1}/5)...`);
            await page.waitForTimeout(2000);
          }
        }
        ...
        if (!subtitleVisible) {
          ...
          await expect(
            subtitleVisible,
            `ETRAINING-STAGE subtitle missing: ${cert.subtitle}\nSubtitles found: ${
              summary.actualSubtitles.length > 0 ? summary.actualSubtitles.join(', ') : 'none'
            }`
          ).toBeTruthy();
        } else {
          console.log(`✅ ETRAINING-STAGE Subtitle Found: ${cert.subtitle}`);
        }
      }

      // Test selecting ONE certification only
      console.log(`ETRAINING-STAGE Testing selection of: ${summary.expectedCertifications[0]}`);
      ...
      console.log(`✅ ETRAINING-STAGE Selected: ${summary.expectedCertifications[0]}`);

      // STOP HERE - NO MORE STEPS
      console.log('✅ ETRAINING-STAGE Certification dropdown validation complete - STOPPING HERE');

      // Take screenshot
      await page.screenshot({ path: 'report/etraining-stage-certification-dropdown-validated.png' });

      console.log('ETRAINING-STAGE Test completed - no registration attempted');
      summary.status = 'passed';
    } catch (error) {
      summary.status = 'failed';
      console.error('ETRAINING-STAGE Test failed:', error);
      throw error;
    } finally {
