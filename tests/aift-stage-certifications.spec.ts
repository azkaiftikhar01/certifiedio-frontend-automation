...
const AIFT_STAGE_EXPECTED_CERTIFICATIONS = [
  {
...
test.describe('AIFT-STAGE Environment - Certification Dropdown Validation', () => {
  test('verify AIFT-STAGE certification dropdown shows all expected values', async ({ page }) => {
    // Very flexible timeout - allow up to 5 minutes for slow environments
    test.setTimeout(300000);
    const summary = {
      environment: 'AIFT-STAGE',
      status: 'pending',
      expectedCertifications: AIFT_STAGE_EXPECTED_CERTIFICATIONS.map((cert) => cert.title),
      expectedSubtitles: AIFT_STAGE_EXPECTED_CERTIFICATIONS.map((cert) => cert.subtitle),
    ...
    const summaryPath = path.resolve('report', 'aift-stage-summary.json');
    ...
    console.log('AIFT-STAGE: Navigating to registration page...');
    ...
    try {
      console.log('AIFT-STAGE: Filling personal information...');
      ...
      console.log('AIFT-STAGE: Opening certification dropdown...');
      ...
      console.log('AIFT-STAGE Environment - Verifying certification dropdown values...');

      // Wait for certifications to load using polling approach (more reliable than networkidle)
      console.log('AIFT-STAGE: Waiting for certifications to load...');
      ...
      while (!certificationsLoaded && (Date.now() - startTime) < maxWaitTime) {
        ...
        if (validTitles.length > 0) {
          console.log(`AIFT-STAGE: Found ${validTitles.length} certifications after ${Math.round((Date.now() - startTime) / 1000)}s`);
      ...
      if (!certificationsLoaded) {
        throw new Error(
          'AIFT-STAGE certifications did not load: no valid <h3> titles found in dropdown within 180s. ' +
      ...
      const collectCertifications = async (retries = 5) => {
        for (let i = 0; i < retries; i++) {
          ...
          if (i < retries - 1) {
            console.log(`AIFT-STAGE: Retrying certification collection (attempt ${i + 1}/${retries})...`);
            await page.waitForTimeout(2000);
          }
        }
      };
      ...
      console.log(`📋 AIFT-STAGE - Found ${summary.actualCertifications.length} certification titles in dropdown`);
      console.log(`📋 AIFT-STAGE - Titles: ${summary.actualCertifications.join(', ')}`);
      console.log(`📋 AIFT-STAGE - Subtitles: ${summary.actualSubtitles.join(', ')}`);
      ...
      for (const cert of AIFT_STAGE_EXPECTED_CERTIFICATIONS) {
        console.log(`AIFT-STAGE: Checking for certification: ${cert.title}`);
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
          ...
          if (attempt < 4) {
            console.log(`AIFT-STAGE: Retrying title check for ${cert.title} (attempt ${attempt + 1}/5)...`);
            await page.waitForTimeout(2000);
          }
        }
        ...
        if (titleCount === 0) {
          ...
          await expect(
            titleLocator,
            `AIFT-STAGE certification missing: ${cert.title}\nTitles found: ${
          ...
          console.log(`✅ AIFT-STAGE Found: ${cert.title}`);
        }
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
          ...
          if (attempt < 4) {
            console.log(`AIFT-STAGE: Retrying subtitle check for ${cert.subtitle} (attempt ${attempt + 1}/5)...`);
            await page.waitForTimeout(2000);
          }
        }
        ...
        if (!subtitleVisible) {
          ...
          await expect(
            subtitleVisible,
            `AIFT-STAGE subtitle missing: ${cert.subtitle}\nSubtitles found: ${
              summary.actualSubtitles.length > 0 ? summary.actualSubtitles.join(', ') : 'none'
            }`
          ).toBeTruthy();
        } else {
          console.log(`✅ AIFT-STAGE Subtitle Found: ${cert.subtitle}`);
        }
      }

      // Test selecting ONE certification only
      console.log(`AIFT-STAGE Testing selection of: ${summary.expectedCertifications[0]}`);
      ...
      console.log(`✅ AIFT-STAGE Selected: ${summary.expectedCertifications[0]}`);

      // STOP HERE - NO MORE STEPS
      console.log('✅ AIFT-STAGE Certification dropdown validation complete - STOPPING HERE');

      // Take screenshot
      await page.screenshot({ path: 'report/aift-stage-certification-dropdown-validated.png' });

      console.log('AIFT-STAGE Test completed - no registration attempted');
      summary.status = 'passed';
    } catch (error) {
      summary.status = 'failed';
      console.error('AIFT-STAGE Test failed:', error);
      throw error;
    } finally {
